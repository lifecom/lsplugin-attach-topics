{if $aAttachTopic}
    <section class="block block-type-stream">
        <header class="block-header">
            <h3>{$aLang.plugin.attachtopics.block_title}</h3>
        </header>
        <div class="block-content">
            <ul class="latest-list">
                {foreach from=$aAttachTopic item=oTopic}
                    ...
                    <li title="{$oTopic->getText()|strip_tags|trim|truncate:150:'...'|escape:'html'}">
                        ...
                        <a href="{$oBlog->getUrlFull()}"
                           class="stream-blog">{$oBlog->getTitle()|escape:'html'}</a> &rarr;
                        <a href="{$oTopic->getUrl()}" class="stream-topic">{$oTopic->getTitle()|escape:'html'}</a>
                        <span class="block-item-comments"><i
                                    class="icon-synio-comments-small"></i>{$oTopic->getCountComment()}</span>
                    </li>
                {/foreach}
            </ul>
        </div>
